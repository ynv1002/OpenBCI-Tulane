...
This folder is a one-off audit and trial-count review workflow for Yaniv's `Yaniv-LRJ(6)-4:7.csv` recording.

This review path remains subject-specific, but the Yaniv file now also participates in the shared offline LRJ benchmark at `analysis/run_lrj_offline_benchmark.py`.

This wrapper itself should still be treated as a review workflow, not a live artifact path.

## Dataset Assumptions

- Marker meanings follow the same LRJ formula used for Ben:
  - `1 = LEFT`
  - `2 = RIGHT`
  - `3 = JAW`
...
The script writes to `analysis/yaniv-lrj-review/outputs/` by default:
...
- This folder is Yaniv-specific on purpose.
- The shared LRJ mechanics now live in `analysis/lrj_review_core.py`, but this wrapper keeps the Yaniv command, docs, and outputs separate from Ben.
...
- Do not add this file to `analysis/utils.py` dataset families from this workflow.
