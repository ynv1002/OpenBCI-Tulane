...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="append",
        default=None,
        help="Restrict the comparison to the listed LR session filename(s). Defaults to the main-scorecard files.",
    )
...
def _run_feature_config(
    ...
    for test_file in include_files:
...
def _build_summary(
    ...
    lines = [
        ...
        "",
        "## Setup",
        f"- Files used: `{', '.join(include_files)}`",
        f"- Evaluation: `leave-one-session-out cross-session` across `{len(include_files)}` run(s)",
        (
            f"- Labeled events: `{int(pooled['test_samples'].iloc[0])} total` "
            f"(across the pooled cross-session evaluation rows)"
        ),
...
def run_feature_comparison(
    ...
    print(f"  Files used: {', '.join(resolved_files)}")
