...
def _resolve_default_lr_input_dir() -> Path:
    root = PROJECT_ROOT.parent / "OPENBCI_runs" / "Yaniv"
    legacy = root / "EEG_LR"
    if legacy.exists():
        return legacy
    return root
...
DEFAULT_INPUT_DIR = _resolve_default_lr_input_dir()
...
def _iter_csvs(input_dir: Path) -> list[Path]:
    csv_paths = sorted(path for path in input_dir.glob("LR-*.csv") if path.is_file())
    if not csv_paths:
        _fail(f"No LR-*.csv files found in {input_dir}")
    return csv_paths
